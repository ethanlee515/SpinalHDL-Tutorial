...
object CpuDemo extends App {
  ...
  case class SimTop() extends Component {
    val pluginsArea = getPlugins()
    val cpu = Cpu(pluginsArea.plugins)
    val mp = pluginsArea.mp

    val simLogic = Fiber setup new Area {
      Fiber.awaitBuild()
      mp.logic.mem.simPublic
    }
  }

  SpinalVerilog(Cpu(getPlugins().plugins))
}
...
object Ex7 extends App {
  ...
  case class WhiteboxerPlugin() extends FiberPlugin {
    val logic = during setup new Area {
      ...
      val instFetch = pp.fetch(INSTRUCTION)
      instFetch.simPublic()
      val mp = host[MemPlugin]
  ...
  SimConfig.compile{
    ...
    // set data of the mem in simulation
    val wp = dut.pluginsArea.wp.logic
    val mem = wp.mem
    mem.setBigInt(address = 1, data = 123)

    val instFetch = wp.instFetch
    
    for(i <- 0 until 3) {
      println(s"instFetch: ${instFetch.toBigInt}")
      cd.waitRisingEdge()
    }
  }
}